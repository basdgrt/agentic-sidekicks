#!/bin/bash
################################################################################
# Claude Code Agentic Sidekicks Setup Script
################################################################################
#
# PURPOSE:
#   This script installs custom agents and skills for Claude Code by creating
#   symbolic links (symlinks) from this repository to Claude's configuration
#   directory.
#
...
#   3. Creates the Claude configuration directory if needed
#   4. Creates symbolic links so Claude Code can access your custom content:
#      - Links agents/ → ~/.claude/agents/agentic-sidekicks
#      - Links skills/ → ~/.claude/commands/agentic-sidekicks
#   5. Verifies the symlinks were created correctly
#
# WHY SYMLINKS?
#   Symlinks allow Claude Code to access your agents/skills directly from this
#   repository. Any changes you make here are immediately available in Claude
...
# Where Claude Code looks for custom agents and commands
...
# Target symlink paths in Claude's config
AGENTS_LINK="$CONF_DIR/agents/agentic-sidekicks"
COMMANDS_LINK="$CONF_DIR/commands/agentic-sidekicks"
...
main() {
    ...
    echo ""
    echo -e "${BOLD}===============================================================${NC}"
    echo -e "${BOLD}  Claude Code Agentic Sidekicks Setup${NC}"
    echo -e "${BOLD}===============================================================${NC}"
    echo ""
    ...
    ensure_parent_directory "$CONF_DIR" "Claude config directory"
    ensure_parent_directory "$CONF_DIR/agents" "Agents parent directory"
    ensure_parent_directory "$CONF_DIR/commands" "Commands parent directory"
    ...
    echo ""
    log_info "Setting up skills as commands..."
    create_symlink "$SKILLS_DIR" "$COMMANDS_LINK" "Commands symlink"
    ...
    verify_symlink "$COMMANDS_LINK" "$SKILLS_DIR" "Commands symlink"
    ...
    echo "  - Commands available at: $COMMANDS_LINK"
    echo ""
    echo -e "${BOLD}Next Steps:${NC}"
    echo "  1. Restart Claude Code if it's currently running"
    echo "  2. Your custom agents are available as sub-agents in Claude Code"
    echo "  3. Your skills are available as /commands in Claude Code"
    echo "  4. Any changes you make in this repository will be immediately available"
    echo ""
